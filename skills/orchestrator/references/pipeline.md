...
3. **Knowledge Graph** — Entity and relationship data; compatible with Neo4j/GraphDB and canonical schema in `src/contracts/intelligence.ts`.
...
- **Canonical** types: `EntityReference`, `Observation`, `Relationship`, `IntelligenceEvent` in `src/contracts/intelligence.ts`. All pipeline outputs that feed the app must map to these types.
